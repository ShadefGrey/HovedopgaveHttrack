...
public class RecursiveCrawlExporter implements Exporter {
    ...
    public void process(File cacheDir) {
        ...
        for (File f1 : cacheChildrens) {
            //TODO trying a non zipped one (should look for "new.zip")
...
//                    pathtest = filename.toString();
                    filename = filename.replace("\\", "/"); //TODO ændrer \ til /
                    filename = filename.substring(filename.indexOf("new.zip") + 8);
